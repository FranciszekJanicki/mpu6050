...
//     mpu6050_set_sampling_rate(mpu6050, 200, DLPF::BW_42); // 1 / (1 + 4) = 200 Hz
//     mpu6050_set_external_frame_sync(mpu6050, ExtSync::TEMP_OUT_L);
//     mpu6050_set_dlpf_mode(mpu6050, DLPF::BW_42);
//     mpu6050_set_full_scale_gyro_range(mpu6050, GYRO_RANGE);
//     mpu6050_write_memory_block(mpu6050, mpu6050_img, sizeof(mpu6050_img), 0x00, 0x00);

//     uint8_t mpu6050_update[] = {0x00, 0x01};
//     mpu6050_write_memory_block(mpu6050, dmp_update, sizeof(dmp_update), 0x02, 0x16);
//     mpu6050_set_dmp_config1(mpu6050, 0x03);
...
// mpu6050_err_t mpu6050_get_dmp_packet(mpu6050_t const* mpu6050, mpu6050_dmp_packet_t* packet)
// {
...
//     if (prefetch_enabled)
//         data |= 0x40;
//     mpu6050_bus_write(mpu6050, MPU6050_REG_ADDRESS_BANK_SEL, data);
// }
...
//         mpu6050_bus_read(mpu6050, MPU6050_REG_ADDRESS_MEM_R_W, data + i, chunk_size);
//         i += chunk_size;
//         address += chunk_size;
...
//         mpu6050_bus_write(mpu6050, MPU6050_REG_ADDRESS_MEM_R_W, data + i, chunk_size);
//         i += chunk_size;
//         address += chunk_size;
...
//         if (length > 0) {
//             mpu6050_write_memory_block(mpu6050, data + i, length, bank, offset);
//             i += length;
//         } else {
//             if (data[i++] == 0x01) {
//                 mpu6050_bus_write(mpu6050, MPU6050_REG_ADDRESS_INT_ENABLE, 0x32);
//             }
//         }
//     }
// }
